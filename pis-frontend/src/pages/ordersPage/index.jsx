...
import { Container, Row, Col, Nav, Form, ListGroup, Button, Card } from 'react-bootstrap';
...
const OrdersPage = () => {
  ...
  const dummyFoodList = [
    ...
    { id: 9, name: "Beer", category: "Beverages", price: 45, weight: "500 ml", description: "Description for Item 9" }
  ];
  ...
  const handleCheckboxChange = (index) => {
    setCheckedItems(prevItems => {
      ...
      const checkedIndexes = Object.keys(updatedItems).filter((index) => updatedItems[index]);
      const allSelectedFood = checkedIndexes.map((index) => orderedFood[selectedTable][index]);
  ...
  const handleCheckout = () => {
    const checkedIndexes = Object.keys(checkedItems).filter((index) => checkedItems[index]);
    const allSelectedFood = checkedIndexes.map((index) => orderedFood[selectedTable][index]);
    setCheckedItems([]);
    alert("Food to checkout: " + allSelectedFood.map(food => food.name).join(", "));
  };
  ...
  const handleRemove = () => {
    const updatedOrderedFood = orderedFood[selectedTable].filter((food, index) => !checkedItems[index]);
    setOrderedFood({
      ...orderedFood,
  ...
  return (
    <>
      {/* Navigation bar for private pages */}
      <PrivateHeaderBar/>

      <Container fluid className="staff-main-container" style={{ paddingTop: navbarHeight }}>
        <Row className="panel">
          {/*Left side - tables */}
              ...
              <Nav className="flex-column">
                ...
                {tables.map((table) => (
                  <Nav.Item key={table} className="nav-table-item">
                    <Nav.Link className="nav-table-link" active={selectedTable === table} onClick={() => handleTableSelect(table)}>
                      Table {table}
                    </Nav.Link>
                  </Nav.Item>
                ))}
          ...
          <Col sm={9} className="p-0">
            <div className="right-panel">
              ...
              {selectedTable && showAddFoodForm ? (
                <>
                  <h2>Ordered Food for <span className="table-title">Table {selectedTable}</span></h2>
                  <div className="order-list-box">
                    <div className="order-list">
                      <ListGroup>
                        {orderedFood[selectedTable].map((food, index) => (
                          <ListGroup.Item key={index} action onClick={() => handleCheckboxChange(index)} active={checkedItems[index]}>
                            <Row>
                              <Col md={6}>
                                {food.name}
                              </Col>
                  ...
                  <div className="div-buttons">
                    <Button variant="primary" type="submit" className="btn-add-food" onClick={handleAddFood}>
                      <FaPlus/>
                    </Button>
                    <Button variant="primary" type="submit" className="btn-add-food" onClick={handleCheckout}>
                      Checkout
                    </Button>
                    <Button variant="primary" type="submit" className="btn-add-food" onClick={handleCheckAll}>
                      <FaRegCircleCheck className="pb-1"/>&nbsp;all
                    </Button>
                    <Button variant="primary" type="submit" className="btn-add-food" onClick={handleUncheckAll}>
                      <FaRegCircleXmark className="pb-1"/>&nbsp;all
                    </Button>
                    <Button variant="primary" type="submit" className="btn-add-food" onClick={handleRemove}>
                      <FaTrash/>
                    </Button>
                  </div>
                </>
                ...
                <>
                  ...
                  {!selectedTable && <div className="null-text">Please select a table to view ordered food</div>}
                        ...
                        <Row>
                          {foodList.map((food, index) => (
                            <Col key={index} md={6} lg={4} xl={3} className="mb-3">
                              {/* Food item */}
                              <Card className="food-card h-100" onClick={() => handleFoodClick(food)}>
                                <Card.Body>
