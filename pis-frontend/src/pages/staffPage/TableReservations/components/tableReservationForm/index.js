  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      <Form.Group controlId="formTables">
        <Form.Label>Select Table(s)</Form.Label>
        <Dropdown>
          <Dropdown.Toggle variant="primary" id="dropdown-tables">
            {selectedTables.length === 0 ? 'Select Tables' : `${selectedTables.length} Table(s) selected`}
          </Dropdown.Toggle>
          <Dropdown.Menu>
            {/* Assuming tables is an array of table objects */}
            {tables.map((table) => (
              <Dropdown.Item
                key={table.id}
                onClick={() => setSelectedTables([...selectedTables, table])}
              >
                {table.description}
              </Dropdown.Item>
            ))}
